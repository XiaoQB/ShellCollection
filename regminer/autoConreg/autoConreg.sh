#!/bin/bash
# 将autoConreg文件夹放入项目同级目录，传项目上级路径为参数
# 结果在autoConreg中
# 先生成repo4conreg，后分割csv，再运行jar包
path=${1}
# path="/mnt/sdb"
...
echo "-------All Repo's csv and meta got!-------"



for repo in `ls -l $path/repo4conreg/ |grep ^d|awk '{print $NF}'`
do
    cd $path/repo4conreg/$repo
    ...
    echo "${repo} regression.csv trimed"

    java -jar $path/autoConreg4j/conreg4j.jar $path $repo
    echo "${repo} finished"
